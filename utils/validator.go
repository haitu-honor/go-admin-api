...
func Verify(st interface{}, roleMap Rules) (err error) {
	...
	typ := reflect.TypeOf(st)  // 返回哪个包的什么类型，例如(main.user)
	val := reflect.ValueOf(st) // 返回值

	kd := val.Kind()          // 获取到st对应的数据类别
	if kd != reflect.Struct { // 判断kd是否是Struct类型
		return errors.New("expect struct")
	}
	num := val.NumField() // 属性个数
	// 遍历结构体的所有字段
	for i := 0; i < num; i++ {
		tagVal := typ.Field(i) // 返回每个成员属性的信息
		val := val.Field(i)    // 返回每个成员值
